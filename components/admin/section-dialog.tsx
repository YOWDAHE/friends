...
export function SectionDialog({ open, onOpenChange, section, onSave }: SectionDialogProps) {
  const [formData, setFormData] = useState({ name: "", imageUrl: "/placeholder.svg" })
  ...
  useEffect(() => {
    if (section) {
      ...
      setFormData({ name: "", imageUrl: "/placeholder.svg" })
    }
    setSelectedFileName("")
  }, [section, open])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    onSave({ ...formData, items: section?.items || [] })
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            {section ? "Update the section details." : "Create a new section within this category."}
          </DialogDescription>
        </DialogHeader>
